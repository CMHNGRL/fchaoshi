<template>
 ...
 <div class="main-container">
      <div class="container-top">
        <ul class="all">
          <li :class="active == 1?'active item':active" @click="handleNewCourse">最新课程</li>
          ...
          <ul class="courseUl">
            ...
            >
              <div class="courseInfo">
                <router-link :to="{path:'/course/'+item._id}">
                  <div class="courseBg">
...
<script>
import {getCourseAllType,courseSearch} from "@/request/course";
...
export default {
  ...
  methods: {
    ...
    //获取课程信息
    courseSearch() {
      courseSearch(this.queryParams).then(res => {
        console.log(res)
    ...
    courseSort(queryParams) {
      courseSearch(queryParams).then(res => {
         this.arrcourse = res.message;
        //将请求回来的数据total赋值给总条数。做分页功能的处理
        this.totalpage =res.status
      })
    ...
    handleNewCourse() {
      ...
      //重新按第一页去请求
      this.queryParams.pageNum = 1;
      this.queryParams.entity.sortBy='addTime'
      this.courseSort(this.queryParams)
    ...
    //分页器事件，实现和courseSearch一样，这里用于区分
    getList(){
      courseSearch(this.queryParams).then(res =>{
        //更新课程列表
        this.arrcourse = res.message;
